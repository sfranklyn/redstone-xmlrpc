...
public class XmlRpcClient extends XmlRpcParser implements XmlRpcInvocationHandler {
     ...
     *
     * @param streamMessages Indicates whether or not to stream messages
     * directly or if the messages should be completed locally before being sent
     * all at once. Streaming is not directly supported by XML-RPC, since the
     * Content-Length header is not included in the HTTP post. If the other end
     * is not relying on Content-Length, streaming the message directly is much
     * more efficient.
     ...
     *
     * @param requestProperties The HTTP request properties to use for future
     * invocations made using this XmlRpcClient instance. These will replace any
     * previous properties set using this method or the setRequestProperty()
     * method.
     */
     ...
     * unserializable. That is, the built-in serializer connot parse it or find
     * a custom serializer that can. There may also be problems with the socket
     * communication.
     ...
     * that the XmlRpcClient instance retains ownership of this map and the map
     * contents is replaced on the next request. If there is a need to keep the
     * fields between requests the map returned should be cloned.
     */
     ...
     *
     * @param arguments The arguments for the call. This may be either a
     * java.util.List descendant, or a java.lang.Object[] array.
     *
     * @param callback An object implementing the XmlRpcCallback interface. If
     * callback is null, the call will be performed but any results, faults, or
     * exceptions will be ignored (fire and forget).
     */
     ...
     * unparsable, this is interpreted as an error in communication and will
     * result in an XmlRpcException.<p>
     *
     ...
     *
     * @throws IOException If a socket error occurs, or if the XML returned is
     * unparsable. This exception is currently also thrown if a HTTP response
     * other than "200 OK" is received.
     * @throws XmlRpcFault
     */
     ...
     *
     * @throws IOException If a connection could not be opened. The exception is
     * propagated out of any unsuccessful calls made into the internal
